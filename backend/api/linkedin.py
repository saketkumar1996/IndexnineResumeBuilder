...
from core.auth import set_session_cookie, SESSION_SECURE, SAME_SITE
...
OAUTH_STATE_COOKIE_NAME = "linkedin_oauth_state"
...
async def linkedin_auth():
    ...
    # Random per-request state, bound to the browser via a short-lived cookie, so the
    # callback can reject codes/state that didn't originate from this browser's flow
    # (prevents OAuth login-CSRF / session fixation).
...
async def linkedin_callback(
    ...
    if error:
        ...
        if state:
            params["state"] = state
        response = RedirectResponse(f"{FRONTEND_REDIRECT_URL}/signin?{urlencode(params)}", status_code=302)
        response.delete_cookie(OAUTH_STATE_COOKIE_NAME, path="/api/linkedin")
        return response

    if not expected_state or not secrets.compare_digest(state or "", expected_state):
        params = {
            "linkedin_error": "invalid_state",
            "linkedin_error_description": "OAuth state mismatch. Please try signing in again.",
        }
        response = RedirectResponse(f"{FRONTEND_REDIRECT_URL}/signin?{urlencode(params)}", status_code=302)
        response.delete_cookie(OAUTH_STATE_COOKIE_NAME, path="/api/linkedin")
        return response
